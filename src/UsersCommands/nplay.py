...
rand_enjo_music = ['🎶 Enjoy!', '🎙️ Vibe!', '🎸 Minor?']
...
class nMusicPlay(commands.Cog):
    ...
    class MusicControls(disnake.ui.View):
        ...
        @disnake.ui.button(label="Pause", emoji="⏸️", style=disnake.ButtonStyle.blurple)
        async def pause_button_callback(self, button, interaction):
            await interaction.response.defer()
            await self.music_cog.pause(interaction, send_message=False)

        @disnake.ui.button(label="Play", emoji="▶️", style=disnake.ButtonStyle.green)
        async def unpause_button_callback(self, button, interaction):
            await interaction.response.defer()
            await self.music_cog.unpause(interaction, send_message=False)

        @disnake.ui.button(label="Skip", emoji="⏭️", style=disnake.ButtonStyle.red)
    ...
    async def play_song(self, ctx, query=None):
        try:
                    ...
                    if query.startswith('http://') or query.startswith('https://'):
                        ...
                        if 'entries' in info:  # It's a playlist
                            ...
                            for entry in info['entries']:
                                self.music_queue.append(entry['webpage_url'])  # Убедитесь, что это правильный URL
            ...
            if voice_client:
                ...
                voice_client.play(source, after=lambda e: self.bot.loop.create_task(self.disconnect_from_voice(ctx)))
                self.last_song_url = url
                minutes, seconds = divmod(duration, 60)
                duration_formatted = f"{minutes} минут {seconds} секунд"
                embed = disnake.Embed(
                    title=f"🎵 {title}",
                    description=f"**Длительность:** {duration_formatted}\n[Ссылка на видео]({video_url})",
                    color=0xfac6d9
                )
    ...
    async def play_next_song(self, interaction):
        if self.music_queue:
            ...
            await self.disconnect_from_voice(interaction.guild)
